...
void tty_report(word_t cyc)
{
    ...
    sim_log(
        "E: instr = %s, valC = 0x%llx, valA = 0x%llx, valB = 0x%llx\n   srcA = %s, srcB = %s, dstE "
        "= %s, dstM = %s, Stat = %s\n",
        ...
        reg_name(id_ex_curr->deste), reg_name(id_ex_curr->destm), stat_name(id_ex_curr->status));

    sim_log(
        "M: instr = %s, Cnd = %d, valE = 0x%llx, valA = 0x%llx\n   dstE = %s, dstM = %s, Stat = "
        "%s\n",
...
static byte_t sim_step_pipe(word_t max_instr, word_t ccount)
{
    ...
    update_pipes();
    tty_report(ccount);
    if (pc_state->op == P_ERROR) pc_curr->status = STAT_PIP;
    if (if_id_state->op == P_ERROR) if_id_curr->status = STAT_PIP;
    if (id_ex_state->op == P_ERROR) id_ex_curr->status = STAT_PIP;
    if (ex_mem_state->op == P_ERROR) ex_mem_curr->status = STAT_PIP;
    if (mem_wb_state->op == P_ERROR) mem_wb_curr->status = STAT_PIP;
...
word_t sim_run_pipe(word_t max_instr, word_t max_cycle, byte_t *statusp, cc_t *ccp)
{
    ...
    while (icount < max_instr && ccount < max_cycle) {
        ...
        if (run_status != STAT_BUB) icount++;
        if (run_status != STAT_AOK && run_status != STAT_BUB) break;
        ccount++;
    }
...
// int *tclDummyMathPtr = (int *)math_error;
...
void do_if_stage()
{
    ...
    */
    ...
    if (!instr_valid) sim_log("\tFetch: Instruction code 0x%llx invalid\n", instr);
...
void do_mem_stage()
{
    ...
    if (read) {
        ...
        if (!dmem_error) sim_log("\tMemory: Read 0x%llx from 0x%llx\n", valm, mem_addr);
    }
    if (mem_write) {
        word_t sink;
        /* Do a read of address just to check validity */
        dmem_error = dmem_error || !get_word_val(mem, mem_addr, &sink);
        if (dmem_error) sim_log("\tMemory: Invalid address 0x%llx\n", mem_addr);
    }
