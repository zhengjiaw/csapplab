...
0000000000400efc <phase_2>: 
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx
  400efe:	48 83 ec 28          	sub    $0x28,%rsp  !# 开10个4个字节的空间，多开4个防止溢出？
  400f02:	48 89 e6             	mov    %rsp,%rsi   !# 将这些地址 传递给 read_six_numbers
                                    !# 所以我们一共传递了 %rdi和%rsi 但是%rsi 实际上是由5个变量的地址
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers> 
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)      !# 第一个参数是否为1？ 是则跳到 400f30
  400f0e:	74 20                	je     400f30 <phase_2+0x34>
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax !# 取得上一个输入的数
  400f1a:	01 c0                	add    %eax,%eax       !# 加倍
  400f1c:	39 03                	cmp    %eax,(%rbx)     !# 与当前数比较
  400f1e:	74 05                	je     400f25 <phase_2+0x29>  !# 相等就跳过下一行
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>    
  400f25:	48 83 c3 04          	add    $0x4,%rbx              !# 下一个参数的位置
  400f29:	48 39 eb             	cmp    %rbp,%rbx  !# 判断是否是第六个参数 0x18 == 24 == 4 * 6
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>  !# 不是则继续循环上面的步骤
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>  !# string输出正确，准备退出
...
0000000000400f43 <phase_3>:
  ...
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi !# 第二个参数 "%d %d"
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  400f60:	83 f8 01             	cmp    $0x1,%eax      
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>  !# 大于1个参数 跳过下一行
  ...
  400fb9:	b8 37 01 00 00       	mov    $0x137,%eax       !# case 1 or default
...
0000000000400fce <func4>:  !# 在分析中只要使 edi == ecx 就能返回正确答案first
  400fce:	48 83 ec 08          	sub    $0x8,%rsp   !# 开8个字节
  ...
  400fdb:	01 c8                	add    %ecx,%eax   !# eax = eax + 0xe
  400fdd:	d1 f8                	sar    %eax        !# SAR EAX,1 eax = 0111 = 0x7
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx  !# ecx = 1 * rsi + rax =0x7
  400fe2:	39 f9                	cmp    %edi,%ecx  
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>  !# edi <= ecx
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx      !# edx = 6 
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>     
  400fee:	01 c0                	add    %eax,%eax
  400ff0:	eb 15                	jmp    401007 <func4+0x39>
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax
  400ff7:	39 f9                	cmp    %edi,%ecx         !# edi >= ecx
...
000000000040100c <phase_4>:
  40100c:	48 83 ec 18          	sub    $0x18,%rsp  !# 24字节
  ...
  401029:	83 f8 02             	cmp    $0x2,%eax      !#  只能是读出两个int 否则就爆炸
  40102c:	75 07                	jne    401035 <phase_4+0x29>
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp) !# 第一个参数first和0xe比较 
  401033:	76 05                	jbe    40103a <phase_4+0x2e>  !# (unsigned)first <= 0xe 否则就爆炸   
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx            !# 第三个参数 edx 
  40103f:	be 00 00 00 00       	mov    $0x0,%esi            !# 第二个参数 esi
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi       !# first 第一个参数 edi
  401048:	e8 81 ff ff ff       	callq  400fce <func4>
  40104d:	85 c0                	test   %eax,%eax            !# 返回值不是0 就爆炸， 否则就成功
  40104f:	75 07                	jne    401058 <phase_4+0x4c>
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)       !# 第二个参数必须是0
...
0000000000401204 <fun7>:      !# 最初 rdi = 0x6030f0   *0x6030f0 = 36
  401204:	48 83 ec 08          	sub    $0x8,%rsp
  401208:	48 85 ff             	test   %rdi,%rdi         !# 是NULL就返回
  40120b:	74 2b                	je     401238 <fun7+0x34>
  40120d:	8b 17                	mov    (%rdi),%edx       !# 取rdi值
  40120f:	39 f2                	cmp    %esi,%edx         !# 值和我们的输入进行比较 unsigned
  401211:	7e 0d                	jle    401220 <fun7+0x1c> !# edx <= 我们的值就去 401220
  401213:	48 8b 7f 08          	mov    0x8(%rdi),%rdi     !# 否则就取其 左儿子 递归
  401217:	e8 e8 ff ff ff       	callq  401204 <fun7>
  40121c:	01 c0                	add    %eax,%eax           !# eax 递增
  40121e:	eb 1d                	jmp    40123d <fun7+0x39>
                        !# edx <= 我们的值 
  401220:	b8 00 00 00 00       	mov    $0x0,%eax       !# 否则 rax 为0
  401225:	39 f2                	cmp    %esi,%edx       !#  相等就返回  
  401227:	74 14                	je     40123d <fun7+0x39>    !# 否则  edx < 我们的值
  401229:	48 8b 7f 10          	mov    0x10(%rdi),%rdi        !# 取其右儿子 递归
  40122d:	e8 d2 ff ff ff       	callq  401204 <fun7>
  401232:	8d 44 00 01          	lea    0x1(%rax,%rax,1),%eax !# eax = 1[rax + rax * 1] = 1
...
0000000000401242 <secret_phase>:
  401242:	53                   	push   %rbx
  401243:	e8 56 02 00 00       	callq  40149e <read_line> !# 读入下一行
  401248:	ba 0a 00 00 00       	mov    $0xa,%edx
  40124d:	be 00 00 00 00       	mov    $0x0,%esi
  401252:	48 89 c7             	mov    %rax,%rdi           !#  rdi = rax = 0, 说明是10进制且base 为0
  401255:	e8 76 f9 ff ff       	callq  400bd0 <strtol@plt> !# 得到 long int
  40125a:	48 89 c3             	mov    %rax,%rbx           !#  rbx 是我们输入的int
  40125d:	8d 40 ff             	lea    -0x1(%rax),%eax
  401260:	3d e8 03 00 00       	cmp    $0x3e8,%eax         !# 1000
  401265:	76 05                	jbe    40126c <secret_phase+0x2a>   !# 必须得 <= 1000 unsigned 
  401267:	e8 ce 01 00 00       	callq  40143a <explode_bomb>        !# 所以我们的输入范围是[1,999]
  40126c:	89 de                	mov    %ebx,%esi                    !# 我们输入的int 是rsi
  40126e:	bf f0 30 60 00       	mov    $0x6030f0,%edi
  401273:	e8 8c ff ff ff       	callq  401204 <fun7>
  401278:	83 f8 02             	cmp    $0x2,%eax            !# 返回值是2 就成功
...
000000000040145c <read_six_numbers>:             !# 第一个参数是%rdi
  40145c:	48 83 ec 18          	sub    $0x18,%rsp    !# 开3个 int64_t
  401460:	48 89 f2             	mov    %rsi,%rdx     !# 第 3 个参数
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx !# 第 4 个参数
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)  !# 第 8 个参数
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax
  401474:	48 89 04 24          	mov    %rax,(%rsp)    !# 第 7 个参数
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9 !# 第 6 个参数
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8 !# 第 5 个参数
                                       !# "%d %d %d %d %d %d"
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi !# 第 2 个参数
  401485:	b8 00 00 00 00       	mov    $0x0,%eax
  40148a:	e8 61 f7 ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  40148f:	83 f8 05             	cmp    $0x5,%eax   !# 也就是说可以输入超过5个的数字
